...
Uso de cl�usulas HAVING y WHERE en la misma consulta
...
-- en el que la instancia de SQL Server se est� ejecutando. El ajuste de zona horaria no est� incluido.
select GETDATE(),* from [@CP_LOGOF] where U_DocEntry=48784 order by U_CT

Este art�culo se tradujo de forma manual. Mueva el puntero sobre las frases del art�culo para ver el texto original. M�s informaci�n.
Traducci�n Original
GETDATE (Transact-SQL)

SQL Server 2014 Otras versiones 
Devuelve la marca de tiempo del sistema de base de datos actual como un valor datetime sin el ajuste de zona horaria de la base de datos. Este valor se deriva del sistema operativo del equipo donde la instancia de SQL Server se est� ejecutando.
Nota Nota
SYSDATETIME y SYSUTCDATETIME tienen m�s precisi�n de fracciones de segundo que GETDATE y GETUTCDATE. SYSDATETIMEOFFSET incluye el ajuste de zona horaria del sistema. SYSDATETIME, SYSUTCDATETIME y SYSDATETIMEOFFSET pueden asignarse a una variable de cualquier tipo de fecha y hora.
Para obtener informaci�n general acerca de todos los tipos de datos y funciones de fecha y hora de Transact-SQL, vea Tipos de datos y funciones de fecha y hora (Transact-SQL).
Se aplica a: SQL Server (SQL Server 2008 a versi�n actual), Windows Azure SQL Database (Versi�n inicial a versi�n actual).
Icono de v�nculo a temas Convenciones de sintaxis de Transact-SQL
Sintaxis
 GETDATE ( )
...
hacer referencia a una expresi�n datetime.
GETDATE es una funci�n no determinista. 
Las vistas y las expresiones que hacen referencia a esta funci�n en una columna no se pueden indizar.

Usar SWITCHOFFSET con la funci�n GETDATE() puede hacer que la consulta se ejecute lentamente debido
a que el optimizador de consultas no puede obtener estimaciones de cardinalidad precisas para el valor
de GETDATE. Se recomienda calcular previamente el valor de GETDATE y despu�s especificar el valor
en la consulta como se muestra en el ejemplo siguiente.

Adem�s, se debe usar la sugerencia de consulta OPTION (RECOMPILE) para obligar al optimizador de
consultas a que vuelva a compilar un plan de consulta la pr�xima vez que se ejecute la misma consulta. 
De este modo, el optimizador tendr� estimaciones de cardinalidad precisas para GETDATE() y generar�
un plan de consulta m�s eficaz.
...
sus fracciones de segundo podr�an ser diferentes.
